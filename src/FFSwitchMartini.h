...
public:
	...
	virtual void CalcSub(double& en, double& vir, const double distSq,
			const uint kind1, const uint kind2) const;
	virtual double CalcEn(const double distSq, const uint kind1,
...
inline void FF_SWITCH_MARTINI::CalcAdd(double& en, double& vir,
		const double distSq, const uint kind1, const uint kind2) const {
...
inline void FF_SWITCH_MARTINI::CalcAdd_1_4(double& en, const double distSq,
	...
	en += epsilon_cn_1_4[index]
			* (sign_1_4[index] * (r_n + shiftRep)
...
inline double FF_SWITCH_MARTINI::CalcEn(const double distSq, const uint kind1,
	...
	double Eij = epsilon_cn[index]
			* (sign[index] * (r_n + shiftRep) - sig6[index] * (r_6 + shiftAtt));
	return Eij;
}
...
inline double FF_SWITCH_MARTINI::CalcVir(const double distSq, const uint kind1,
	...
	double Wij = epsilon_cn[index]
			* (sign[index] * (n_ij * r_n2 + dshiftRep)
...
inline void FF_SWITCH_MARTINI::Calc(double & en, double & vir,
	...
	vir = epsilon_cn[index]
			* (sign[index] * (n * r_n2 + dshiftRep)
					- sig6[index] * (6.0 * r_8 + dshiftAtt));
