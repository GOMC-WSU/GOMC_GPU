...
struct Cryst1: FWReadableBase {
	...
	XYZArray axis;
	Cryst1(void) :
			currBox(0), hasVolume(false), axis(BOX_TOTAL) {
	}
...
struct Atoms: FWReadableBase {
	...
	std::vector<uint> box;
	std::vector<std::string> atomAliases, resNamesFull, resNames, resKindNames;
	...
	Atoms(void) :
			restart(false), currBox(0), count(0), currRes(10) {
	}
	void SetRestart(config_setup::RestartSettings const& r);
	void SetBox(const uint b) {
		currBox = b;
