...
class Reader {
public:
	Reader(std::string const& name, std::string const& alias, bool useSkipW =
			false, std::string const* const skipW = NULL, bool useSkipC = false,
			std::string const* const skipC = NULL, const bool crit = true,
			const bool note = true) :
			firstVal(""), skipChars(""), skipWordsEnable(false), skipCharsEnable(
					false) {
