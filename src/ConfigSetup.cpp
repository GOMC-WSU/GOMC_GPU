...
void ConfigSetup::Init(const char *fileName) {
	...
	while (reader.readNextLine(line)) {
		if (line[0] == "Restart") {
			...
			if (boxnum >= BOX_TOTAL) {
				std::cout << "Error: This simulation requires only "
						<< BOX_TOTAL << " number of PDB file(s)!" << std::endl;
				exit(0);
			}
			...
			if (boxnum >= BOX_TOTAL) {
				std::cout << "Error: This simulation requires only "
						<< BOX_TOTAL << " number of PSF file(s)!" << std::endl;
				exit(0);
			}
			...
			if (box < BOX_TOTAL) {
				...
				std::cout << "Error: This simulation requires only "
						<< BOX_TOTAL << " number of box dimension(s)!"
						<< std::endl;
				exit(0);
			}
		}
...
void ConfigSetup::fillDefaults(void) {
	if (sys.exclude.EXCLUDE_KIND == UINT_MAX) {
		std::cout << "Warning: By default value (1-3) for exclude is selected!"
				<< std::endl;
		sys.exclude.EXCLUDE_KIND = sys.exclude.EXC_ONETHREE_KIND;
	}

	if (in.ffKind.numOfKinds == 0) {
		std::cout << "Error: Force field type has not been defined!"
				<< std::endl;
		exit(0);
	}

	if (in.prng.kind == "") {
		std::cout << "Warning: By default, random seed has been selected!"
				<< std::endl;
	...
	if (sys.gemc.kind == UINT_MAX) {
		std::cout << "Warning: By default, GEMC-NVT has been selected!"
				<< std::endl;
		sys.gemc.kind = mv::GEMC_NVT;
	}
#endif
	...
	if (sys.ewald.enable && sys.ewald.oneFourScale == DBL_MAX) {
		std::cout << "Warning: 1-4 electro static scaling has been set to zero!"
				<< std::endl;
...
void ConfigSetup::verifyInputs(void) {
	...
	if (sys.gemc.kind == mv::GEMC_NPT && sys.gemc.pressure == DBL_MAX) {
		std::cout << "Error: Pressure has not been specified for GEMC-NPT!"
				<< std::endl;
		exit(0);
	}
	if (sys.gemc.kind == mv::GEMC_NVT && sys.gemc.pressure != DBL_MAX) {
		std::cout << "Warning: Pressure will be ignored for GEMC-NVT!"
				<< std::endl;
	}
#endif
	...
	if (in.prng.kind == "RANDOM" && in.prng.seed != UINT_MAX) {
		std::cout << "Warning: Seed will not be used for RANDOM type seed!"
				<< std::endl;
	}
	if (in.prng.kind == "INTSEED" && in.prng.seed == UINT_MAX) {
		std::cout << "Error: Seed is required for INTSEED type seed!"
				<< std::endl;
		exit(0);
	}
	if (in.ffKind.numOfKinds != 1) {
		std::cout << "Error: One type of Parameter type should be set!"
				<< std::endl;
		exit(0);
	}
	if (in.files.param.name == "") {
		std::cout << "Error: Parameter file name has not been defined!"
				<< std::endl;
	}
	if (sys.ff.VDW_KIND == UINT_MAX) {
		std::cout << "Error: Potential type has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.ff.VDW_KIND == sys.ff.VDW_STD_KIND && sys.ff.doTailCorr == false) {
		std::cout
				<< "Warning: Long Range Correction has been disabled for standard VDW potential type!"
				<< std::endl;
	}
	if ((sys.ff.VDW_KIND == sys.ff.VDW_SHIFT_KIND
			...
			&& sys.ff.doTailCorr) {
		std::cout
				<< "Warning: Long Range Correction will be disabled for shift and switch potential!"
				<< std::endl;
	}
	...
	if (sys.ewald.enable && (sys.ewald.alpha == DBL_MAX)) {
		std::cout
				<< "Error: Alpha value has not been specified for Ewald summation!"
				<< std::endl;
		exit(0);
	}
	if (sys.ewald.enable && (sys.ewald.KMax == DBL_MAX)) {
		std::cout
				<< "Error: KMax value has not been specified for Ewald summation!"
				<< std::endl;
		exit(0);
	}
	...
	if (sys.step.adjustment == ULONG_MAX) {
		std::cout << "Error: Adjustment steps has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.step.equil == ULONG_MAX) {
		std::cout << "Error: Equilibrium steps has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.step.total == ULONG_MAX) {
		std::cout << "Error: Total run steps has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.step.adjustment > sys.step.equil) {
		std::cout
				<< "Error: Adjustment steps should be smaller than Equilibrium steps!"
				<< std::endl;
		exit(0);
	}
	if (sys.step.equil > sys.step.total) {
		std::cout
				<< "Error: Equilibrium steps should be smaller than Total run steps!"
				<< std::endl;
		exit(0);
	}
	if (sys.moves.displace == DBL_MAX) {
		std::cout << "Error: Displacement frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.moves.rotate == DBL_MAX) {
		std::cout << "Error: Rotation frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
#if ENSEMBLE == GEMC
	if (sys.moves.volume == DBL_MAX) {
		std::cout << "Error: Volume swap frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.moves.transfer == DBL_MAX) {
		std::cout << "Error: Molecule swap frequency has not been specified!"
				<< std::endl;
		exit(0);
	}

	if (fabs(
			(sys.moves.displace + sys.moves.rotate + sys.moves.transfer
					+ sys.moves.volume) - 1.0f) > 1.0e-4) {
	...
	if (sys.ff.VDW_KIND != sys.ff.VDW_STD_KIND
		...
		std::cout << "Error: Switch cut off should be smaller than cut off!"
				<< std::endl;
		exit(0);
	}
#ifdef VARIABLE_PARTICLE_NUMBER
	if (sys.cbmcTrials.bonded.ang == UINT_MAX) {
		std::cout
				<< "Error: CBMC number of angle trials has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.cbmcTrials.bonded.dih == UINT_MAX) {
		std::cout
				<< "Error: CBMC number of dihedral trials has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.cbmcTrials.nonbonded.first == UINT_MAX) {
		std::cout
				<< "Error: CBMC number of first site trials has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (sys.cbmcTrials.nonbonded.nth == UINT_MAX) {
		std::cout
				<< "Error: CBMC number of nth site trials has not been specified!"
				<< std::endl;
		exit(0);
	}
#endif
	...
	if (out.state.settings.enable && out.state.settings.frequency == ULONG_MAX) {
		std::cout
				<< "Error: Coordinates output frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (out.restart.settings.enable
			&& out.restart.settings.frequency == ULONG_MAX) {
		std::cout
				<< "Error: Restart coordinate output frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (out.console.enable && out.console.frequency == ULONG_MAX) {
		std::cout << "Error: Console output frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (out.statistics.settings.block.enable
			&& out.statistics.settings.block.frequency == ULONG_MAX) {
		std::cout
				<< "Error: Block average output frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (out.statistics.settings.fluct.enable
			&& out.statistics.settings.fluct.frequency == ULONG_MAX) {
		std::cout
				<< "Error: Fluctuation output frequency has not been specified!"
				<< std::endl;
		exit(0);
	}
	if (out.console.frequency == ULONG_MAX) {
		...
		std::cout
				<< "Warning: By default console output frequency has been set to "
				<< out.console.frequency << "!" << std::endl;
	}
	if (out.restart.settings.frequency == ULONG_MAX) {
		out.restart.settings.frequency = (ulong) sys.step.total;
		std::cout
				<< "Warning: By default restart coordinate output frequency has been set to "
				<< out.restart.settings.frequency << "!" << std::endl;
	}
	if (out.state.settings.frequency == ULONG_MAX) {
		out.state.settings.frequency = (ulong) sys.step.total / 10;
		std::cout
				<< "Warning: By default coordinate output frequency has been set to "
				<< out.state.settings.frequency << "!" << std::endl;
	}
	if (out.statistics.settings.block.frequency == ULONG_MAX) {

		out.statistics.settings.block.frequency = (ulong) sys.step.total / 100;
		std::cout
				<< "Warning: By default block average output frequency has been set to "
				<< out.statistics.settings.block.frequency << "!" << std::endl;
	}
	if (out.statistics.settings.fluct.frequency == ULONG_MAX) {
		if (sys.step.total > 1000) {
			out.statistics.settings.fluct.frequency = (ulong) sys.step.total
					/ 1000;
		} else {
			out.statistics.settings.fluct.frequency = (ulong) sys.step.total
					/ 100;
		...
		std::cout
				<< "Warning: By default fluctuation output frequency has been set to "
				<< out.statistics.settings.fluct.frequency << "!" << std::endl;
	}
#if ENSEMBLE == GCMC
	...
	if (!out.statistics.settings.block.enable
		...
		std::cout
				<< "Warning: Output block average has been set off, energy output for block average will be ignored!"
				<< std::endl;
	}
	if (!out.statistics.settings.block.enable
			&& out.statistics.vars.pressure.block) {
		std::cout
				<< "Warning: Output block average has been set off, pressure output for block average will be ignored!"
				<< std::endl;
	}
#ifdef VARIABLE_PARTICLE_NUMBER
	if (!out.statistics.settings.block.enable
		...
		std::cout
				<< "Warning: Output block average has been set off, molecule number output for block average will be ignored!"
				<< std::endl;
	}
	if (!out.statistics.settings.block.enable
			&& out.statistics.vars.acceptAngles.block) {
		std::cout
				<< "Warning: Output block average has been set off, accept angles output for block average will be ignored!"
				<< std::endl;
	}
#endif
#ifdef VARIABLE_DENSITY
	if (!out.statistics.settings.block.enable
		...
		std::cout
				<< "Warning: Output block average has been set off, density output for block average will be ignored!"
				<< std::endl;
	}
#endif
#ifdef VARIABLE_VOLUME
	if (!out.statistics.settings.block.enable
			&& out.statistics.vars.volume.fluct) {
		std::cout
				<< "Warning: Output block average has been set off, volume ouput for block average will be ignored!"
				<< std::endl;
	}
#endif
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.energy.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, energy ouput for fluctuation will be ignored!"
				<< std::endl;
	}
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.pressure.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, pressure ouput for fluctuation will be ignored!"
				<< std::endl;
	}
#ifdef VARIABLE_PARTICLE_NUMBER
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.molNum.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, molecule number ouput for fluctuation will be ignored!"
				<< std::endl;
	}
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.acceptAngles.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, accept angles ouput for fluctuation will be ignored!"
				<< std::endl;
	}
#endif
#ifdef VARIABLE_DENSITY
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.density.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, density ouput for fluctuation will be ignored!"
				<< std::endl;
	}
#endif
#ifdef VARIABLE_VOLUME
	if (!out.statistics.settings.fluct.enable
			&& out.statistics.vars.volume.fluct) {
		std::cout
				<< "Warning: Output fluctuation has been set off, volume ouput for fluctuation will be ignored!"
				<< std::endl;
	}
#endif
}
...
const std::string config_setup::PRNGKind::KIND_RANDOM = "RANDOM",
		...
		config_setup::FFValues::VDW = "VDW", config_setup::FFValues::VDW_SHIFT =
				"VDW_SHIFT", config_setup::FFValues::VDW_SWITCH = "VDW_SWITCH",
