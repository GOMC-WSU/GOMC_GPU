...
FFParticle::FFParticle() :
		mass(NULL), nameFirst(NULL), nameSec(NULL), n(NULL), n_1_4(NULL), sigmaSq(
				NULL), sigmaSq_1_4(NULL), epsilon_cn(NULL), epsilon_cn_1_4(
				NULL), epsilon_cn_6(NULL), epsilon_cn_6_1_4(NULL), nOver6(NULL), nOver6_1_4(
				NULL), enCorrection(NULL), virCorrection(NULL), shiftConst(
				NULL), An(NULL), Bn(NULL), Cn(NULL), shiftConst_1_4(NULL), An_1_4(
				NULL), Bn_1_4(NULL), Cn_1_4(NULL), sig6(NULL), sign(NULL), sig6_1_4(
				NULL), sign_1_4(NULL), rCut(0), rCutSq(0), rOnSq(0), rOn(0), A6(
				0), B6(0), C6(0), factor1(0), factor2(0) {
}
...
void FFParticle::Init(ff_setup::Particle const& mie,
		ff_setup::NBfix const& nbfix, config_setup::FFValues const& val,
	...
	if (vdwKind == val.VDW_SWITCH_KIND && isMartini) {
		...
		rOn = val.rswitch;
		rOnSq = rOn * rOn;
		A6 = 6.0 * ((6.0 + 1) * rOn - (6.0 + 4) * rCut)
				/ (pow(rCut, 6.0 + 2) * pow(rCut - rOn, 2));
		B6 = -6.0 * ((6.0 + 1) * rOn - (6.0 + 3) * rCut)
				/ (pow(rCut, 6.0 + 2) * pow(rCut - rOn, 3));
		C6 = 1.0 / pow(rCut, 6.0) - A6 / 3.0 * pow(rCut - rOn, 3)
				- B6 / 4.0 * pow(rCut - rOn, 4);
	}
	if (vdwKind == val.VDW_SWITCH_KIND && !isMartini) {
		rOn = val.rswitch;
		rOnSq = rOn * rOn;
		...
		for (uint j = 0; j < size; j++) {
			if (nbfix.name[i] == nameFirst[j] || nbfix.name[i] == nameSec[j]) {
				...
				if (vdwKind == num::VDW_SWITCH_KIND && isMartini) {
					double pn = n[j];
					An[j] = pn * ((pn + 1) * rOn - (pn + 4) * rCut)
							/ (pow(rCut, pn + 2) * pow(rCut - rOn, 2));
					Bn[j] = -pn * ((pn + 1) * rOn - (pn + 3) * rCut)
							/ (pow(rCut, pn + 2) * pow(rCut - rOn, 3));
					Cn[j] = 1.0 / pow(rCut, pn)
							- An[j] / 3.0 * pow(rCut - rOn, 3)
					...
					An_1_4[j] = pn_1_4
							* ((pn_1_4 + 1) * rOn - (pn_1_4 + 4) * rCut)
							/ (pow(rCut, pn_1_4 + 2) * pow(rCut - rOn, 2));
					Bn_1_4[j] = -pn_1_4
							* ((pn_1_4 + 1) * rOn - (pn_1_4 + 3) * rCut)
							/ (pow(rCut, pn_1_4 + 2) * pow(rCut - rOn, 3));
					Cn_1_4[j] = 1.0 / pow(rCut, pn_1_4)
							- An_1_4[j] / 3.0 * pow(rCut - rOn, 3)
...
void FFParticle::Blend(ff_setup::Particle const& mie, const double rCut) {
	for (uint i = 0; i < count; ++i) {
		...
		for (uint j = 0; j < count; ++j) {
			...
			double cn = n[idx] / (n[idx] - 6)
					* pow(n[idx] / 6, (6 / (n[idx] - 6)));
			...
			tc *= 0.5 * 4.0 * M_PI;
			epsilon_cn[idx] = cn * num::MeanG(mie.epsilon, mie.epsilon, i, j);
			epsilon_cn_1_4[idx] = cn
					* num::MeanG(mie.epsilon_1_4, mie.epsilon_1_4, i, j);
			...
			if (vdwKind == num::VDW_SWITCH_KIND && isMartini) {
				double pn = n[idx];
				An[idx] = pn * ((pn + 1) * rOn - (pn + 4) * rCut)
						/ (pow(rCut, pn + 2) * pow(rCut - rOn, 2));
				Bn[idx] = -pn * ((pn + 1) * rOn - (pn + 3) * rCut)
						/ (pow(rCut, pn + 2) * pow(rCut - rOn, 3));
				Cn[idx] = 1.0 / pow(rCut, pn)
						- An[idx] / 3.0 * pow(rCut - rOn, 3)
				...
				Bn_1_4[idx] = -pn_1_4
						* ((pn_1_4 + 1) * rOn - (pn_1_4 + 3) * rCut)
						/ (pow(rCut, pn_1_4 + 2) * pow(rCut - rOn, 3));
				Cn_1_4[idx] = 1.0 / pow(rCut, pn_1_4)
						- An_1_4[idx] / 3.0 * pow(rCut - rOn, 3)
