...
void Remarks::Read(FixedWidthReader & pdb) {
	...
	ulong readStep;
	std::string varName;
	pdb.Get(remNum, rem_num::POS).Get(varName, name::POS).Get(readStep,
			data::POS);
...
void Remarks::CheckStep(std::string const& varName, const ulong readStep) {
	using namespace pdb_entry::remark::field;
...
#ifndef NDEBUG
	if (reached && restart)
		std::cout << "Restart step " << restartStep << " reached." << std::endl;
#endif
}
...
void Remarks::CheckGOMC(std::string const& varName) {
	...
	if (!str::compare(varName, name::STR_GOMC)) {
		std::cerr << "ERROR: Restart failed, " << "GOMC file's identifying tag "
				<< "\"REMARK  1   GOMC\" is missing" << std::endl << std::endl;
		exit(1);
	}
}
...
void Atoms::Read(FixedWidthReader & file) {
	...
	file.Get(atomName, field::alias::POS).Get(resName, field::res_name::POS).Get(
			resNum, field::res_num::POS).Get(l_chain, field::chain::POS).Get(
			l_x, field::x::POS).Get(l_y, field::y::POS).Get(l_z, field::z::POS).Get(
			l_occ, field::occupancy::POS);
	...
	for (uint b = 0; b < BOX_TOTAL; b++) {
		...
		while (pdb.Read(varName, pdb_entry::label::POS)) {
			...
			if (dataKind != dataKinds.end()
					&& (remarks.reached
							|| str::compare(dataKind->first,
									pdb_entry::label::REMARK))) {
