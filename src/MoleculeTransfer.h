...
inline void MoleculeTransfer::CalcEn()
{
   if (ffRef.useLRC)
   {
      ...
      {
	double recipNew = calcEwald.SwapDestRecip(newMol, destBox);
	double recipNew1 = calcEwald.SwapSourceRecip(oldMol, sourceBox);
	 recipGain.energy = recipNew -
	   sysPotRef.boxEnergy[destBox].recip;
	 
	 recipLose.energy = recipNew1 -
	   sysPotRef.boxEnergy[sourceBox].recip;
