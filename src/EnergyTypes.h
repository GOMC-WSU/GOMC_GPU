...
struct Elect{
	...
	__host__ __device__ Elect():virial(0.0), energy(0.0){}
	__host__ __device__ Elect(const double vir, const double en) :
	...
	//VALUE SETTER
		...
		//OPERATORS
		__host__  __device__ Elect& operator=(Elect const& rhs) {
			virial = rhs.virial;
			energy = rhs.energy;
			return *this;
		}
...
struct Intermolecular {
	...
	//CONSTRUCTORS
	__host__ __device__ Intermolecular() :
			virial(0.0), energy(0.0) {
	}
...
struct Energy {
	...
	Energy() :
			intraBond(0.0), intraNonbond(0.0), inter(0.0), tc(0.0), real(0.0), recip(0.0), self(0.0),
			correction(0.0), elect(0.0), total(0.0) {
	}

	Energy(double bond, double nonbond, double inter, double real, double recip, double self, double correction) :
			intraBond(bond), intraNonbond(nonbond), inter(inter), tc(0.0), real(real), recip(recip),
			self(self), correction(correction), elect(real+recip+self+correction), total(0.0) {
	}
	...
	__host__  __device__ Energy& operator-=(Elect const& rhs) {
		elect -= rhs.energy;
		return *this;
	}
	__host__  __device__ Energy& operator+=(Elect const& rhs) {
...
struct Virial {
	...
	__host__  __device__ Virial& operator-=(Elect const& rhs) {
		elect -= rhs.virial;
		return *this;
	}
	__host__  __device__ Virial& operator+=(Elect const& rhs) {
...
struct SystemPotential {
	...
	__host__ __device__ void Add(const uint b, double en, double vir){
		boxVirial[b].inter += vir;
		boxEnergy[b].inter += en;
	}

	__host__ __device__ void AddReal(const uint b, double* envir){
//		boxVirial[b].real += envir[1];
		boxEnergy[b].real += envir[0];
	}

	__host__ __device__ void AddRecip(const uint b, double* envir){
//		boxVirial[b].recip += envir[1];
		boxEnergy[b].recip += envir[0];
	}
...
inline std::ostream& operator <<(std::ostream& out, const Energy& en) {
	out << "Total: " << en.total << "   Inter: " << en.inter << "   IntraB: "
			<< en.intraBond << "   IntraNB: " << en.intraNonbond << '\n';
	return out;
}
#endif
