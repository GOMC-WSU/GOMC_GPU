...
void MoleculeLookup::TotalAtomsMols(uint * numByBox, uint * numByKindBox,
		uint * numAtomsByBox, MoleculeKind * kinds) const {
...
void MoleculeLookup::TotalAndDensity(uint * numByBox, uint * numByKindBox,
	...
	for (uint b = 0; b < BOX_TOTAL; ++b) {
		...
		if (numKinds > 1) {
			for (uint k = 0; k < numKinds; ++k) {
				if (sum > 0) {
					molFractionByKindBox[mkIdx2] = numByKindBox[mkIdx2]
							* invBoxTotal;
				} else {
