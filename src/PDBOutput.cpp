...
void PDBOutput::InitPartVec(pdb_setup::Atoms const& atoms)
{
 ...
 {
	 // copy back GPU DATA
	 // Get the coords from the GPU
	/* int ctr =0;
	 MoleculeLookup::box_iterator tm = molLookupRef.BoxBegin(0);
	 MoleculeLookup::box_iterator en = molLookupRef.BoxEnd(1);
	 while(tm != en)
	 {
		 for(uint i = 0; i <  sysRef->calcEnergy.mols.GetKind(*tm).NumAtoms(); ++i) 
		 {
			 sysRef->coordinates.x[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.Gpu_x[ctr ];
			 sysRef->coordinates.y[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.Gpu_y[ctr ];
			 sysRef->coordinates.z[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.Gpu_z[ctr ];
			 ctr++;
		 }
		 ++tm;
	 }
*/
   ...
   cordsx= (double *)malloc (sizeof(double) * sysRef->calcEnergy.currentCoords.Count());
   cordsy= (double *)malloc (sizeof(double) * sysRef->calcEnergy.currentCoords.Count());
   cordsz= (double *)malloc (sizeof(double) * sysRef->calcEnergy.currentCoords.Count());
   cudaMemcpy(cordsx,sysRef->calcEnergy.Gpu_x,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   cudaMemcpy(cordsy,sysRef->calcEnergy.Gpu_y,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   cudaMemcpy(cordsz,sysRef->calcEnergy.Gpu_z,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   ...
   int ctr =0;
      ...
      while(tm != en)
      {
            ...
            for(uint i = 0; i <  sysRef->calcEnergy.mols.GetKind(*tm).NumAtoms(); ++i) 
            {
             	sysRef->coordinates.x[ sysRef->calcEnergy.mols.start[*tm] + i ] = cordsx[ctr ];
				sysRef->coordinates.y[ sysRef->calcEnergy.mols.start[*tm] + i ] = cordsy[ctr ];
				sysRef->coordinates.z[ sysRef->calcEnergy.mols.start[*tm] + i ] = cordsz[ctr ];
                ctr++;
            }
         ++tm;
      }
