...
PDBOutput::PDBOutput(System & sys, StaticVals const& statV) :
		moveSetRef(sys.moveSettings), molLookupRef(sys.molLookupRef), coordCurrRef(
				sys.coordinates), pStr(coordCurrRef.Count(),
				GetDefaultAtomStr()), boxDimRef(sys.boxDimRef), molRef(
				statV.mol) {
...
void PDBOutput::Init(pdb_setup::Atoms const& atoms,
	...
	if (enableOutState) {
		for (uint b = 0; b < BOX_TOTAL; ++b) {
			...
			outF[b].Init(output.state.files.pdb.name[b], aliasStr, true,
					notify);
			outF[b].open();
		}
	...
	if (enableRestOut) {
		for (uint b = 0; b < BOX_TOTAL; ++b) {
			...
			outRebuildRestartFName[b].replace(
					...
					outRebuildRestartFName[b].end(), newStrAddOn);
			outRebuildRestart[b].Init(outRebuildRestartFName[b], aliasStr, true,
					notify);
		}
...
void PDBOutput::InitPartVec(pdb_setup::Atoms const& atoms) {
	...
	for (uint b = 0; b < BOX_TOTAL; ++b) {
		MoleculeLookup::box_iterator m = molLookupRef.BoxBegin(b), end =
				molLookupRef.BoxEnd(b);
		while (m != end) {
			uint mI = *m;
...
void PDBOutput::GetDataFromGPU() {
	...
	cudaMemcpy(sysRef->calcEnergy.cordsx, sysRef->calcEnergy.Gpu_x,
			sizeof(double) * sysRef->calcEnergy.currentCoords.Count(),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(sysRef->calcEnergy.cordsy, sysRef->calcEnergy.Gpu_y,
			sizeof(double) * sysRef->calcEnergy.currentCoords.Count(),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(sysRef->calcEnergy.cordsz, sysRef->calcEnergy.Gpu_z,
			sizeof(double) * sysRef->calcEnergy.currentCoords.Count(),
			cudaMemcpyDeviceToHost);
	...
	MoleculeLookup::box_iterator en = sysRef->molLookupRef.BoxEnd(
			BOX_TOTAL - 1);
	while (tm != en) {

		for (uint i = 0; i < sysRef->calcEnergy.mols.GetKind(*tm).NumAtoms();
				++i) {
			sysRef->coordinates.x[sysRef->calcEnergy.mols.start[*tm] + i] =
					sysRef->calcEnergy.cordsx[ctr];
			sysRef->coordinates.y[sysRef->calcEnergy.mols.start[*tm] + i] =
					sysRef->calcEnergy.cordsy[ctr];
			sysRef->coordinates.z[sysRef->calcEnergy.mols.start[*tm] + i] =
					sysRef->calcEnergy.cordsz[ctr];
			ctr++;
		}
		++tm;
	}
