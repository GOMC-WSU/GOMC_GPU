...
public:
	...
	void DeviceQuery(); //  
	void RunDisplaceMove(uint rejectState, uint majKind, uint step); //
	void RunRotateMove(uint rejectState, uint majKind, uint step); //
	...
	SystemPotential NewConvCellListSystemTotalEnergy(uint majKind,
			SystemPotential curpot); // for volume move 
	SystemPotential NewConvCellListSystemTotalEnergyOneBox(uint majKind,
			int bPick); // for NPT 
#endif
