...
Molecules::Molecules() :
		start(NULL), kIndex(NULL), countByKind(NULL), chain(NULL), kinds(NULL), pairEnCorrections(
				NULL), pairVirCorrections(NULL) {
}
...
void Molecules::Init(Setup& setup, Forcefield& forcefield, System & sys) {
	...
	for (uint mk = 0; mk < kindsCount; mk++) {
		countByKind[mk] = std::count(atoms.resNames.begin(),
				atoms.resNames.end(), atoms.resKindNames[mk]);
			...
			for (uint pI = 0; pI < kinds[i].NumAtoms(); ++pI) {
				for (uint pJ = 0; pJ < kinds[j].NumAtoms(); ++pJ) {
					pairEnCorrections[i * kindsCount + j] +=
							forcefield.particles->EnergyLRC(
									kinds[i].AtomKind(pI),
									kinds[j].AtomKind(pJ));
					pairVirCorrections[i * kindsCount + j] +=
							forcefield.particles->VirialLRC(
									kinds[i].AtomKind(pI),
									kinds[j].AtomKind(pJ));
				}
			}
			//set other side of the diagonal
			pairEnCorrections[j * kindsCount + i] = pairEnCorrections[i
					* kindsCount + j];
			pairVirCorrections[j * kindsCount + i] = pairVirCorrections[i
					* kindsCount + j];
		}
	}
