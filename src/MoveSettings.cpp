...
void MoveSettings::Update(const bool isAccepted, const uint moveIndex,
	...
	if (moveIndex < mv::SCALEABLE)
		acceptPercent[moveIndex] = (double) (tempAccepted[moveIndex]
				+ accepted[moveIndex])
...
//Adjust responsibly
void MoveSettings::Adjust(const uint majMoveKind, const uint moveIndex,
		const uint b) {
	if (tempTries[moveIndex] > 0) {
		double currentAccept = (double) (tempAccepted[moveIndex])
				/ (double) (tempTries[moveIndex]), fractOfTargetAccept =
				currentAccept / TARGET_ACCEPT_FRACT;
