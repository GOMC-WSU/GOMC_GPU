  ...
  DCDiatomic::DCDiatomic(DCData* data, const mol_setup::MolKind kind,
   ...
   void DCDiatomic::BuildOld(TrialMol& oldMol, uint molIndex)
   {
     ...
     uint nLJTrials = data->nLJTrialsNth;
      ...
      //data->calc.ParticleInter(first, positions, inter, molIndex, oldMol.GetBox(), nLJTrials);
	  
	   data->calc.
		   GetParticleEnergyGPU(
		   oldMol.GetBox(),
		   nLJTrials, 
		   inter,
		   positions,
      ...
      stepWeight = 0;
	   data->axes.WrapPBC(positions, oldMol.GetBox());
     //  data->calc.ParticleInter(second, positions, inter, molIndex,oldMol.GetBox(), nLJTrials);

	   data->calc.GetParticleEnergyGPU(oldMol.GetBox(), nLJTrials, inter,positions, oldMol.molLength, oldMol.mOff, second,oldMol.molKindIndex);
    for (uint trial = 0; trial < nLJTrials; trial++)
      {
         stepWeight += exp(-1 * beta * inter[trial]);
      }
   ...
   void DCDiatomic::BuildNew(TrialMol& newMol, uint molIndex)
   {
     ...
     uint nLJTrials = data->nLJTrialsNth;
      ...
      //  data->calc.ParticleInter(first, positions, inter, molIndex,  newMol.GetBox(), nLJTrials);

	   data->calc.GetParticleEnergyGPU(newMol.GetBox(), nLJTrials, inter,positions, newMol.molLength, newMol.mOff, first,newMol.molKindIndex);//  
      ...
      data->axes.WrapPBC(positions, newMol.GetBox());
    //  data->calc.ParticleInter(second, positions, inter, molIndex,newMol.GetBox(), nLJTrials);

	   data->calc.GetParticleEnergyGPU(newMol.GetBox(), nLJTrials, inter,positions, newMol.molLength, newMol.mOff, second,newMol.molKindIndex);//  
      stepWeight = 0;
     for (uint trial = 0; trial < nLJTrials; trial++)
      {
         ljWeights[trial] = exp(-1 * beta * inter[trial]);
