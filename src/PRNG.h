...
public:
	...
	uint PickMolAndBox(uint & m, uint &mk, uint &b, uint &mOff, double subDraw,
			const double movPerc) //  
			{
		double boxDiv = 0;
		PickBox(b, subDraw, boxDiv, movPerc);
	...
	uint PickMolAndBoxPair(uint &m, uint &mk, uint &mOff, uint & bSrc,
