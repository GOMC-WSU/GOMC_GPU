...
void AssignMolKinds(MolKind& kind, const pdb_setup::Atoms& pdbData,
...
int ReadPSFAtoms(FILE* psf, MolMap& kindMap, uint nAtoms);
...
std::vector<Angle> mol_setup::AtomEndAngles(const MolKind& molKind, uint atom) {
	...
	for (Aiter it = molKind.angles.begin(), end = molKind.angles.end();
			it < end; ++it) {
...
void MolSetup::AssignKinds(const pdb_setup::Atoms& pdbAtoms,
	...
	while (it != kindMap.end()) {
		...
		for (uint i = 0; i < it->second.atoms.size(); i++) {
			std::cout << i << "\t" << it->second.atoms[i].name << '\t'
					<< it->second.atoms[i].type << '\t'
					<< it->second.atoms[i].charge << '\t'
...
int ReadPSFAtoms(FILE* psf, MolMap& kindMap, unsigned int nAtoms) {
	...
	while (atomID < nAtoms) {
		...
		if (it == kindMap.end()) {
			it = kindMap.insert(
					std::make_pair(std::string(moleculeName), MolKind())).first;
		...
		//still building a molecule...
			...
			if (molID != it->second.firstMolID)
				...
				it->second.atoms.push_back(
						Atom(atomName, atomType, charge, mass));
		}
	}
...
int ReadPSFAngles(FILE* psf, MolMap& kindMap,
	...
	for (unsigned int i = 0; i < firstAtom.size(); ++i) {
		...
		while (atom0 < molBegin || atom0 >= molEnd) {
			...
			if (feof(psf) || ferror(psf)) {
				fprintf(stderr,
						"ERROR: Could not find all angles in PSF file ");
				return READERROR;
			}
		}
		//read in angles
		while (atom0 >= molBegin && atom0 < molEnd) {
			currentMol.angles.push_back(
					Angle(atom0 - molBegin, atom1 - molBegin,
							atom2 - molBegin));
			dummy = fscanf(psf, "%u %u %u", &atom0, &atom1, &atom2);
		}
	}
...
int ReadPSFDihedrals(FILE* psf, MolMap& kindMap,
	...
	for (unsigned int i = 0; i < firstAtom.size(); ++i) {
		...
		while (dih.a0 < molBegin || dih.a0 >= molEnd) {
			dummy = fscanf(psf, "%u %u %u %u", &dih.a0, &dih.a1, &dih.a2,
					&dih.a3);
			if (feof(psf) || ferror(psf)) {
				fprintf(stderr,
						"ERROR: Could not find all dihedrals in PSF file ");
				return READERROR;
			}
		...
		while (dih.a0 >= molBegin && dih.a0 < molEnd) {
			...
			if (std::find(currentMol.dihedrals.begin(),
					currentMol.dihedrals.end(), dih)
