...
void Coordinates::TranslateRand(XYZArray & dest, XYZ & newCOM, uint & pStart,
		uint & pLen, const uint m, const uint b, const double max) {
...
void Coordinates::RotateRand(XYZArray & dest, uint & pStart, uint & pLen,
	...
	RotationMatrix matrix = RotationMatrix::FromAxisAngle(prngRef.Sym(max),
			prngRef.PickOnUnitSphere());
...
void Coordinates::VolumeTransferTranslate(uint & state, Coordinates & dest,
		COM & newCOM, BoxDimensions & newDim, COM const& oldCOM,
...
void Coordinates::VolumeTransferTranslate(uint & state, Coordinates & dest,
		COM & newCOM, BoxDimensions & newDim, COM const& oldCOM,
		const double max, double & scaleO, double & scaleN,
		double & randN) const {
...
void Coordinates::TranslateOneBox(Coordinates & dest, COM & newCOM,
		COM const& oldCOM, BoxDimensions const& newDim, const uint b,
		const double scale) const {
	uint pStart = 0, pStop = 0, pLen = 0;
	MoleculeLookup::box_iterator curr = molLookRef.BoxBegin(b), end =
			molLookRef.BoxEnd(b);
	while (curr != end) {
