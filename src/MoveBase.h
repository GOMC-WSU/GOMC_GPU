...
class VolumeTransfer : public MoveBase
{
 public:
   ...
   virtual void Accept(const uint rejectState ,uint step );
   inline void AcceptGPU(const uint rejectState, SystemPotential newPot , SystemPotential curPot, uint bPick, uint step);//  
...
inline uint VolumeTransfer::Transform()
{
   ...
   /*coordCurrRef.VolumeTransferTranslate(state, newMolsPos, newCOMs, newDim, 
					comCurrRef, b_i, b_ii, max);*/

	if(GEMC_KIND == mv::GEMC_NVT)
	{
			coordCurrRef.VolumeTransferTranslate(state, newMolsPos, newCOMs, newDim,
		comCurrRef, b_i, b_ii, max, scaleO, scaleN, randN); 
	}
	else
	{
...
inline void VolumeTransfer::AcceptGPU(const uint rejectState, SystemPotential newPot , SystemPotential curPot, uint bPick,uint step )
		...
		else
		{
			...
			else
			{
				cudaMemcpy(calcEnRef.Gpu_x + calcEnRef.AtomCount[0], 
					...
					sizeof(double) *calcEnRef.AtomCount[1] ,    cudaMemcpyDeviceToDevice);
