...
class TransformMatrix {
public:
	__host__ __device__ TransformMatrix() {
		LoadIdentity();
	}
	__host__ __device__ explicit TransformMatrix(double d) {
		SetDiagonal(d);
	}
	...
	__host__ __device__ inline void Apply(double& xpos, double& ypos,
			double& zpos) const; // 
	...
	__host__  __device__  static TransformMatrix UniformRandom(double u1,
			double u2, double u3);
	...
	__host__ __device__ void BasisRotation(const XYZ& u, const XYZ& v,
			const XYZ& w);

	__host__  __device__ TransformMatrix operator*(
			const TransformMatrix& rhs) const;
...
__host__ __device__ inline void TransformMatrix::AddRotation(
		const XYZ& rotateBy) {
...
__host__ __device__ inline void TransformMatrix::Apply(double& xpos,
		double& ypos, double& zpos) const  // 
		{
	XYZ result;
	result.x = matrix[0][0] * xpos + matrix[0][1] * ypos + matrix[0][2] * zpos;
	result.y = matrix[1][0] * xpos + matrix[1][1] * ypos + matrix[1][2] * zpos;
	result.z = matrix[2][0] * xpos + matrix[2][1] * ypos + matrix[2][2] * zpos;
...
__host__  __device__  inline TransformMatrix TransformMatrix::operator*(
		const TransformMatrix& o) const {
...
__host__  __device__  inline TransformMatrix TransformMatrix::CrossProduct(
		const XYZ& vector) {
...
__host__  __device__  inline TransformMatrix TransformMatrix::TensorProduct(
		const XYZ& u) {
...
__host__  __device__  inline TransformMatrix TransformMatrix::FromAxisAngle(
		double theta, const TransformMatrix& cross,
...
//on [0, 1)
__host__  __device__  inline TransformMatrix TransformMatrix::UniformRandom(
